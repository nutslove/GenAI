...
system_prompt_for_supervisor = (
    ...
    # "You can only execute reference/read-only commands (using execute_command_agent worker) to check the status of resources you need to verify while troubleshooting the issue.\n"
    # "Never execute commands that create, update, delete or reset resources like `aws ecs create-cluster --cluster-name MyCluster --tags key=key1,value=value1`, `aws iam attach-user-policy`, `aws ec2 delete-vpc --vpc-id vpc-a01106c2`.\n"
...
def main():
    ...
    if execute_or_not == "yes":
        execute_command_graph.invoke(
            {
                "messages": [HumanMessage(content=final_state["command"])]
            }
        )
